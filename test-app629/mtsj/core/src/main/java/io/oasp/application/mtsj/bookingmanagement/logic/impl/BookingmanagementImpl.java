...
import io.oasp.application.mtsj.bookingmanagement.dataaccess.api.BookingRepository;
import io.oasp.application.mtsj.bookingmanagement.dataaccess.api.InvitedGuestEntity;
import io.oasp.application.mtsj.bookingmanagement.dataaccess.api.InvitedGuestRepository;
import io.oasp.application.mtsj.bookingmanagement.dataaccess.api.TableEntity;
import io.oasp.application.mtsj.bookingmanagement.dataaccess.api.TableRepository;
...
public class BookingmanagementImpl extends AbstractComponentFacade implements Bookingmanagement {
  ...
  public boolean deleteBooking(Long bookingId) {
    ...
    List<OrderCto> bookingOrders = this.orderManagement.findOrderCtos(criteria).getContent();
  ...
  public boolean deleteInvitedGuest(Long invitedGuestId) {
    ...
    List<OrderCto> guestOrdersCto = this.orderManagement.findOrderCtos(criteria).getContent();
  ...
  public InvitedGuestEto acceptInvite(String guestToken) {
    ...
    InvitedGuestEto invited = findInvitedGuestEtos(criteria).getContent().get(0);
  ...
  public InvitedGuestEto declineInvite(String guestToken) {
    ...
    List<OrderCto> guestOrdersCto = this.orderManagement.findOrderCtos(guestOrderCriteria).getContent();
    for (OrderCto orderCto : guestOrdersCto) {
      this.orderManagement.deleteOrder(orderCto.getOrder().getId());
    }
    BookingCto booking = findBooking(invited.getBookingId());
  ...
  public BookingCto findBookingByEmail(String email) {
    ...
    List<BookingCto> bookings = findBookingCtos(bookingCriteria).getContent();
    if (!bookings.isEmpty()) {
      return bookings.get(0);
    } else {
      InvitedGuestSearchCriteriaTo invitedCriteria = new InvitedGuestSearchCriteriaTo();
      invitedCriteria.setEmail(email);
      List<InvitedGuestEto> inviteds = findInvitedGuestEtos(invitedCriteria).getContent();
      if (!inviteds.isEmpty()) {
  ...
  public void cancelInvite(String bookingToken) {
    ...
    List<BookingCto> booking = findBookingCtos(bookingCriteria).getContent();
    ...
    if (!booking.isEmpty()) {
      ...
      List<InvitedGuestEto> guestsEto = findInvitedGuestEtos(guestCriteria).getContent();
